...
Sources/ascelerate/
  ...
  ProductVersions.swift               # Pending-version detection for IAPs/subs/groups (versions endpoints) + review-item resolution
...
- **[asc-swift](https://github.com/aaronsky/asc-swift)** (1.0.0+) — App Store Connect API client
  ...
  - Resolved version: 1.7.1 (with swift-crypto, URLQueryEncoder, swift-asn1 as transitive deps)
...
- **IAP/sub submissions 409 without a pending version** — `inAppPurchaseSubmissions`/`subscriptionSubmissions`/`subscriptionGroupSubmissions` POSTs return 409 "has no pending version for submission" for products without pending edits. An APPROVED product with pending edits *stays* APPROVED, so `ProductVersions` (ProductVersions.swift) checks the product's version history instead: `pendingIAP`/`pendingSubscription`/`pendingGroup` query the `versions` sub-resource with `filter[state]` = PREPARE_FOR_SUBMISSION, READY_FOR_REVIEW, REJECTED, DEVELOPER_REJECTED (inferred from the app-version convention, not yet observed on a real pending edit — adjust `pendingStates` if it turns out wrong). Used by `submitBundledProducts()` in `apps review submit` (candidates are still READY_TO_SUBMIT/APPROVED products; the specific 409 is still caught per item as a safety net), by `iap submit`/`sub submit`/`sub submit-group` (which no longer require READY_TO_SUBMIT — any pending version qualifies, incl. approved products with edits — and stop before any request otherwise), and by `iap info`/`sub info` (`pendingVersion` in text + JSON, nil-omitted). `ProductVersions.reviewItems` resolves review-submission items to app version / IAP / subscription / group; `apps review status` does this for active submissions only (one extra request per product item) and its JSON items gain optional `kind`, `name`, `version`, `versionState`.
- **Product versions model (ASC OpenAPI 4.4.1 / asc-swift 1.7.1, verified live 2026-09-22)** — IAPs, subscriptions, and subscription groups now expose a version history: `GET /v2/inAppPurchases/{id}/versions`, `/v1/subscriptions/{id}/versions`, `/v1/subscriptionGroups/{id}/versions` (each record: `version` int + `state` from PREPARE_FOR_SUBMISSION/READY_FOR_REVIEW/WAITING_FOR_REVIEW/IN_REVIEW/ACCEPTED/APPROVED/REPLACED_WITH_NEW_VERSION/REJECTED/DEVELOPER_REJECTED; `filter[state]` supported). The list endpoints (`apps/{id}/inAppPurchasesV2`, `apps/{id}/subscriptionGroups`, `subscriptionGroups/{id}/subscriptions`) accept `include: [.versions]` + `limitVersions`, but `relationships.versions.data` only hydrates when `versions` is ALSO named in the parent's `fields[...]` (same gotcha as the IAP price schedule). Review submission items reference bundled products as `inAppPurchaseVersion`/`subscriptionVersion`/`subscriptionGroupVersion` relationships (seen on a real submission), and `ReviewSubmissionItemCreateRequest` accepts them. The new v2 localizations (`/v2/inAppPurchaseLocalizations`, `/v2/subscriptionLocalizations`, `/v2/subscriptionGroupLocalizations`) hang off a version and have NO `state` attribute — state lives on the version. A product's pending edits should therefore be detected via a version in a non-APPROVED/non-REPLACED state rather than via v1 localization states (which miss image/screenshot-only edits). Wired via `ProductVersions.swift` (see the 409 bullet below); there is no DELETE for version records.
...
- **Xcode 27 removed Simulator.app.** Simulator windows are now hosted by `<Xcode>/Contents/Applications/DeviceHub.app` (bundle ID `com.apple.dt.Devices`); Xcode ≤26 ships `<Xcode>/Contents/Developer/Applications/Simulator.app`. `open -a Simulator` resolves by name via LaunchServices and fails once no Simulator.app is registered. `SimulatorManager.launchUIHost()` resolves the host inside the `xcode-select -p` Xcode (DeviceHub first, then Simulator.app, then bundle-ID fallbacks); failure only prints a warning and the run continues headless. **DeviceHub does not open windows for booted devices on its own** (Simulator.app did): launching it only shows its device-management window, `-CurrentDeviceUDID` is ignored, and there is no AppleScript/CLI. The only hook is its URL scheme: `open -a <DeviceHub.app> "devices://device/open?id=<udid>"` opens (or focuses) that device's window — `SimulatorManager.showDeviceWindow(udid:)` calls it after every boot in `prepareDevicesConcurrently` (no-op for Simulator.app). Nothing else depends on the app: every other step goes through `xcrun simctl` (CoreSimulatorService), verified live with DeviceHub on Xcode 27.1 (2026-09-22): boot, status_bar, ui appearance, spawn, io screenshot, and full XCUITest runs.
